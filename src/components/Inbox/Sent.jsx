...
import {sgetConnections2} from '../../Config/ApiUrls';
import { sCardBankrequest2 } from '../../Config/ApiUrls';
...
const [Connections11,setConnections11]=useState([{}]);
const [Connections5,setConnections5]=useState([{}]);
const [banks,setBanks]=useState(0);
const [banks1,setBanks1]=useState(0);

const handleTabs1=(id,id1) => {
  setBanks(id);  
  setBanks1(id1);  
...
const sgetCard1=()=>{
  ...
  urlencoded.append("userId", banks);
  urlencoded.append("bankId", banks1);
  
    axios.post(sCardBankrequest2, urlencoded, 
    {
...
const sgetConnections1=()=>{
    
  const param = new URLSearchParams({userId:"10005"});
    axios.get(sgetConnections2, { 
    params:param,
...
const newgetConnections9=()=>{
  ...
  const param = new URLSearchParams({userId:"10005"});
    ...
    headers:{
      ...
      var card7=response7.data.data;
      
      


setConnections5(card7);
...
var sarrayOfObject = Connections11.map(function (value, index){
  return [value, Connections5[index]]
});
      ...
      search: createSearchParams({
        ...
        image:result3.concat(el.userPhoto)
                ...
                <div className='_MessageDiv121'>
                    ...
                    <div>
                        <p>Sent @ {el.createdAt}</p>
                    </div>
                    <div >
                        <p>{el.note}</p>
                    </div>
                    <div>
                    <button type="button" class="button"  onClick={() => handleTabs1(el.userId,el.bankId)} >Cancel</button>
                    </div>
                </div>
            </div>
        </div>
